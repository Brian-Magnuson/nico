...
const char* EXAMPLE_LIB_PATH = NICO_TESTLIB_EXAMPLE_LIB_PATH;
const char* INTEROP_LIB_PATH = NICO_TESTLIB_INTEROP_LIB_PATH;
...
TEST_CASE("JIT extern block", "[jit]") {
    ...
    SECTION("Extern block with real function") {
        run_jit_test(
            ...
            JITTestOptions{
                .expected_output = "4",
                .static_library_paths = {EXAMPLE_LIB_PATH}
            }
        );
    }
    ...
    SECTION("Extern block with example variable") {
        run_jit_test(
            R"(
            extern "C" example {
                static var examplelib_number: i32
            }
...
TEST_CASE("JIT external linkage declarations", "[jit]") {
    SECTION("External function declaration 1") {
        run_jit_test(
            ...
            JITTestOptions{
                .expected_output = "52",
                .static_library_paths = {INTEROP_LIB_PATH}
            }
        );
    }
