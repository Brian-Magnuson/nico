...
void SymbolTree::install_primitive_types() {
    ...
    new_node = std::make_shared<Node::PrimitiveType>(reserved_scope, "i32", std::make_shared<Type::Int>(true, 32));
    new_node->initialize_node();

    new_node = std::make_shared<Node::PrimitiveType>(reserved_scope, "f64", std::make_shared<Type::Float>(64));
    new_node->initialize_node();

    new_node = std::make_shared<Node::PrimitiveType>(reserved_scope, "bool", std::make_shared<Type::Bool>());
    new_node->initialize_node();
}

std::optional<std::shared_ptr<Node>> SymbolTree::search_name_from_scope(const Name& name, std::shared_ptr<Node::IScope> scope) const {
...
std::pair<std::shared_ptr<Node>, Err> SymbolTree::add_field_entry(const Field& field) {
    if (auto node = reserved_scope->children.at(std::string(field.token->lexeme))) {
        return std::make_pair(node.value(), Err::NameIsReserved);
    }
