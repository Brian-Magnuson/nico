...
bool SymbolTree::register_symbol(
    ...
    if (it != symbol_map.end()) {
        if (is_symbol_autogenerated) {
            // If the autogenerated symbol is already in use, we log a specific
            // error. This happens if the user creates an entry with an
            // unmangled symbol that matches our name mangling scheme.
            Logger::inst().log_error(
                Err::SymbolAlreadyExists,
                node->location,
                "Cannot declare `" + node->short_name +
                    "` here because its symbol `" + the_symbol +
                    "` would conflict with an existing entry."
            );
        }
        else if (!it->second.has_value()) {
            // If the symbol is in the map, but has no node, it means the symbol
            // is reserved.
            Logger::inst().log_error(
        ...
        if (it->second.has_value() &&
            ...
            // If we can locate the previous declaration, log its location as a
            // note.
            auto locatable_node =
...
SymbolTree::add_namespace(std::shared_ptr<Token> token) {
    ...
    if (auto node = current_scope->children.at(std::string(token->lexeme))) {
        ...
        register_symbol(new_node);

        current_scope = new_node;
        modified = true;
        return std::make_pair(new_node, Err::Null);
    }
}
...
SymbolTree::add_overloadable_func(const Field& field) {
    ...
    if (auto node = current_scope->children.at(field.name)) {
        ...
        register_symbol(overload_group);
        modified = true;
    }
    ...
    register_symbol(new_node, custom_symbol);
    overload_group->overloads.push_back(new_node);
    modified = true;

    return {new_node, Err::Null};
}
