...
// TODO: Clean up all commented functions.
// std::shared_ptr<Type>
...
// ) {
//     auto expr_ptr_type =
//     std::dynamic_pointer_cast<Type::IPointer>(expr_type); auto
//     target_ptr_type =
...
//         // This ensures the dictionary has an entry for every parameter in
//         the
//         // correct order.
//     }
...
//         expr->type = std::make_shared<Type::Bool>(); // The result type is
//         Bool.
//     }
//     else {
//         expr->type = l_type; // The result type is the same as the left
//         operand.
//     }
//     return std::any();
// }
...
//         expr->type = std::make_shared<Type::RawTypedPtr>(r_type,
//         expr->has_var);
//     }
...
//         // Note: References have a lot of rules that we can't enforce yet, so
//         we
//         // disallow them for now.
//         panic(
//             "LocalChecker::visit(Expr::Address*): References are not
//             supported " "yet."
//         );
//     }
//     else {
//         panic(
//             "LocalChecker::visit(Expr::Address*): Unknown address-of
//             operator."
//         );
//     }
...
//             expr->op->location,
//             "Cannot create a mutable pointer/reference to an immutable
//             value."
//         );
...
//         // For pointer dereference, the assignability is carried over from
//         the
...
//         );
//         Logger::inst().log_note(
//             "Aggregate type members must be sized to calculate member
//             offsets."
//         );
//         return std::any();
//     }
...
//             // For tuple access, the assignability and possible error
//             location
...
//     if (has_error)
//         return std::any();
//     // Don't worry about the types right now; we'll check them during
//     matching.
...
//             // Due to the semantics of the overloadedfn type, the callee must
//             be
//             // a name reference.
//             if (name_ref) {
...
//     expr->type = std::make_shared<Type::Int>(false, 64); // Sizeof returns
//     u64.
...
//         return std::any();
//     }
//     // You can still do `(alloc expr).property` since it contains a
//     dereference.
...
//         if (!anno_any.has_value())
//             return std::any();
//         auto alloc_inner_type =
//         std::any_cast<std::shared_ptr<Type>>(anno_any);
...
//                 return std::any();
//             }
//             // alloc_inner_type takes precedence over init_type, so we don't
//             do
...
//         return std::any();
//     expr->type =
//         std::make_shared<Type::Array>(first_elem_type,
//         expr->elements.size());
...
//     );
//     if (!ok) {
//         panic("LocalChecker::visit(Expr::Block*): Could not add local
//         scope.");
//     }
