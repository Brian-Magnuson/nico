...
TEST_CASE("Parser expressions", "[parser]") {
    ...
    SECTION("Binary 2") {
        ...
        auto ast = parser.parse(std::move(tokens));
        std::vector<std::string> expected = {
            "(expr (binary + (lit 1) (binary * (lit 2) (lit 3))))", "(stmt:eof)"
        };
        CHECK(printer.stmts_to_strings(ast) == expected);
    }
    ...
    SECTION("Binary 5") {
        ...
        auto ast = parser.parse(std::move(tokens));
        std::vector<std::string> expected = {
            "(expr (binary and (lit true) (lit false)))", "(stmt:eof)"
        };
        CHECK(printer.stmts_to_strings(ast) == expected);
    }
    ...
    SECTION("Binary 7") {
        ...
        auto ast = parser.parse(std::move(tokens));
        std::vector<std::string> expected = {
            "(expr (binary or (lit true) (unary not (lit true))))", "(stmt:eof)"
        };
        CHECK(printer.stmts_to_strings(ast) == expected);
    }
...
TEST_CASE("Parser groupings and tuples", "[parser]") {
    ...
    SECTION("Grouping 3") {
        ...
        auto ast = parser.parse(std::move(tokens));
        std::vector<std::string> expected = {
            "(expr (binary * (binary + (lit 1) (lit 2)) (lit 3)))", "(stmt:eof)"
        };
        CHECK(printer.stmts_to_strings(ast) == expected);
    }
