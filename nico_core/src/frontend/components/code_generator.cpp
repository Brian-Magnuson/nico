...
std::any CodeGenerator::visit(Stmt::Func* stmt) {
    ...
    if (stmt->body.has_value()) {
        ...
        if (Type::is_a<Type::Void>(func_type->return_type)) {
...
std::any CodeGenerator::visit(Expr::Access* expr, bool as_lvalue) {
    ...
    if (Type::is_a<Type::Tuple>(expr->left->type)) {
        ...
        llvm::Value* field_ptr = builder->CreateStructGEP(
            struct_type->get_llvm_type(builder),
            left,
            field_index,
            "struct_field"
        );
...
std::any CodeGenerator::visit(Expr::Call* expr, bool as_lvalue) {
    ...
    if (Type::is_a<Type::Void>(callee_fn_type->return_type)) {
